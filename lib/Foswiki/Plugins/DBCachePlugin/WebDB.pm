...
@Foswiki::Plugins::DBCachePlugin::WebDB::ISA =
...
sub isModified {
    ...
    return 1
      if $this->_getModificationTime() == 0
          || $this->{_loadTime} < $this->_getModificationTime();
    return 0;
}
...
sub onReload {
    ...
    foreach my $topicName (@$topics) {
     ...
     # for some odd reason
        unless ($topic) {
            writeDebug(
"trying to load topic '$topicName' in web '$this->{web}' but it wasn't found in the cache"
            );
            next;
        }
        ...
        # get meta object
        my ( $meta, $text ) =
          Foswiki::Func::readTopic( $this->{web}, $topicName );
        my $origText = $text;
        ...
        # createdate
        my ( $createDate, $createAuthor ) =
          Foswiki::Func::getRevisionInfo( $this->{web}, $topicName, 1 );
...
sub getNeighbourTopics {
    ...
    unless ( $prevTopic && $nextTopic ) {
        ...
        my ($resultList) =
          $this->dbQuery( $theSearch, undef, $theOrder, $theReverse );
        my $state = 0;
...
#print STDERR "DEBUG: called dbQuery($theSearch, $theTopics, $theSort, $theReverse) in $this->{web}\n";
    ...
    my $webViewPermission =
      Foswiki::Func::checkAccessPermission( 'VIEW', $wikiName, undef, undef,
        $this->{web} );
...
# work out howto abstract this concept - or to disable it (its worth about 400mS per topic in the set. (if you're not WikiAdmin))
            if (
                ...
                || Foswiki::Func::checkAccessPermission(
                    'VIEW', $wikiName, undef, $topicName, $this->{web}
                )
              )
            {
                ...
                if ( $theSort =~ /^(on|name)$/ ) {
                    ...
                    $sorting{$topicName} =
                      $this->expandPath( $topicObj, join( " and ", @sorting ) );
                    $doNumericalSort = 0
...
sub expandPath {
    ...
    if ( $thePath =~ /^%/ ) {
        $thePath =
          &Foswiki::Func::expandCommonVariables( $thePath, '', $this->{web} );
        $thePath =~ s/^%/<nop>%/o;
