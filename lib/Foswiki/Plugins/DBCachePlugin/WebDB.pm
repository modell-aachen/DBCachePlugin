...
sub onReload {
    ...
    foreach my $topicName (@$topics) {
     ...
     # for some odd reason
        unless ($topic) {
            print STDERR
"ERROR: trying to load topic '$topicName' in web '$this->{web}' but it wasn't found in the cache\n";
            next;
        }
...
# work out howto abstract this concept - or to disable it (its worth about 400mS per topic in the set. (if you're not WikiAdmin))
              ...
              )
            {
                ...
                if ( $theSort =~ /^(on|name)$/ ) {
                    ...
                    $sorting{$topicName} =
                      $info ? $topicObj->fastget('info')->fastget('date') : 0;
                }
                    ...
                    $theSort =~ s/\$dollar/\$/go;
                    $sorting{$topicName} =
                      $this->expandPath( $topicObj, $theSort );
