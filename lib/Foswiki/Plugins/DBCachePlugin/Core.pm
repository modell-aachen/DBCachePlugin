...
use vars qw(
  ...
  $foswikiWeb $baseWeb $baseTopic %MON2NUM $dbQueryCurrentWeb
);
...
sub init {
    ...
    $foswikiWeb   = $Foswiki::cfg{SystemWebName};
}
...
s/\$formatTime\((.*?)(?:,\s*'([^']*?)')?\)/formatTime($theDB->expandPath($topicObj, $1), $2)/geo
    ...
    if ( defined $theHeader ) {
        ...
        $theHeader =
          Foswiki::Func::expandCommonVariables( $theHeader, $thisTopic,
            $thisWeb );
    }
    if ( defined $theFooter ) {
        ...
        $theFooter =
          Foswiki::Func::expandCommonVariables( $theFooter, $thisTopic,
            $thisWeb );
    }
...
sub handleDBCALL {
    ...
    my $thisWeb;
    ( $thisWeb, $thisTopic ) =
      &Foswiki::Func::normalizeWebTopicName( $baseWeb, $thisTopic );
...
sub handleDBSTATS {
    ...
    $result =
      &Foswiki::Func::expandCommonVariables( $theHeader . $result . $theFooter,
        $thisTopic, $thisWeb );
...
sub handleATTACHMENTS {
    ...
    $theFormat =
'| [[$url][$name]] |  $sizeK | <nobr>$date</nobr> | $wikiuser | $comment |'
    ...
    if ( $theSort eq 'name' ) {
        @attachments =
          sort { lc( $a->fastget('name') ) cmp lc( $b->fastget('name') ) }
          @attachments;
    }
    elsif ( $theSort eq 'date' ) {
        @attachments =
          sort { $a->fastget('date') <=> $b->fastget('date') } @attachments;
    }
    elsif ( $theSort eq 'size' ) {
        @attachments =
          sort { $a->fastget('size') <=> $b->fastget('size') } @attachments;
    }
    elsif ( $theSort eq 'user' ) {
        @attachments =
          sort { $a->fastget('user') cmp $b->fastget('user') } @attachments;
    }
    elsif ( $theSort eq 'comment' ) {
        @attachments =
          sort { lc( $a->fastget('comment') ) cmp lc( $b->fastget('comment') ) }
          @attachments;
    }
    elsif ( $theSort eq 'comment:name' ) {
        @attachments = sort {
            lc( $a->fastget('comment')   || $a->fastget('name') ) cmp
    ...
    foreach my $attachment (@attachments) {
        ...
        if ( $Foswiki::Plugins::VERSION >= 1.2 ) {    # TWiki-4.2 onwards
        ...
        my ( $userWeb, $userTopic ) =
          Foswiki::Func::normalizeWebTopicName( '', $user );
        ...
        # actions
        my $webDavUrl =
          '%WIKIDAVPUBURL%/' . $thisWeb . '/' . $thisTopic . '/' . $name;
        my $webDavAction =
            '<a rel="nofollow" href="'
          . $webDavUrl . '" '
          . 'title="%MATETEXT{"edit [_1] using webdav" args="<nop>'
          . $name . '"}%">'
          . '%MAKETEXT{"edit"}%</a>';

        my $propsUrl =
            '%SCRIPTURLPATH{"attach"}%/' 
          . $thisWeb . '/'
          . $thisTopic
          . '?filename='
          . $name
        ...
        my $propsAction =
            '<a rel="nofollow" href="'
          . $propsUrl . '" '
          . 'title="%MAKETEXT{"manage properties of [_1]" args="<nop>'
          . $name . '"}%">'
          . '%MAKETEXT{"props"}%</a>';

        my $moveUrl =
            '%SCRIPTURLPATH{"rename"}%/' 
          . $thisWeb . '/'
          . $thisTopic
          . '?attachment='
          . $name;
        my $moveAction =
            '<a rel="nofollow" href="' 
          . $moveUrl . '" '
          . 'title="%MAKETEXT{"move or delete [_1]" args="<nop>'
          . $name . '"}%">'
          . '%MAKETEXT{"move"}%</a>';
        ...
        my $deleteAction =
            '<a rel="nofollow" href="'
          . $deleteUrl . '" '
          . 'title="%MAKETEXT{"delete [_1]" args="<nop>'
          . $name . '"}%">'
          . '%MAKETEXT{"delete"}%</a>';
    ...
    $theHeader =
      expandVariables( $theHeader, $thisWeb, $thisTopic, count => $index );
    $theFooter =
      expandVariables( $theFooter, $thisWeb, $thisTopic, count => $index );
...
sub fixInclude {
    ...
    $_[0] =
      $session->{renderer}->takeOutBlocks( $_[0], 'noautolink', $removed );
...
m|([0-9]{1,2})[-\s/]+([A-Z][a-z][a-z])[-\s/]+([0-9]{4})[-\s/]+([0-9]{1,2}):([0-9]{1,2})|
    ...
    # "31 Dec 2003", "31 Dec 03", "31-Dec-2003", "31/Dec/2003"
        ...
        $date =~ m|([0-9]{1,2})[-\s/]+([A-Z][a-z][a-z])[-\s/]+([0-9]{2,4})| )
    {
        $day  = $1;
        $mon  = $MON2NUM{$2} || 0;
        $year = $3;
